...
- `http://myapp.vozi.duckdns.org` 처럼 깔끔한 주소로 로컬 서버를 공유할 수 있습니다.
...
| `--server` | `-s` | Relay 서버 주소 (필수) | `-s http://vozi.duckdns.org:3000` |
...
- **Q. 접속이 안 돼요!**
    ...
    - DuckDNS 도메인이 서버 IP와 정확히 연결되어 있는지 확인하세요. (`ping myapp.vozi.duckdns.org`)
